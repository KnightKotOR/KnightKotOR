...
2. [Audio classification (group project, responsible for ML)](https://github.com/yrmint/ml-app-arch):
   ...
   * Designed and trained a 2 MB CNN for audio classification, achieving **F1 macro = 0.82 without data leakage**; performance is comparable to fine-tuned 380 MB transformers
...
3. [Topological Data Analysis research (group project)](https://github.com/KnightKotOR/topology-mnist3d):
   ...
   * Trained XGBoost with an **F1 macro score of 0.81**, which is about **10% higher than the baseline CNN** (F1 = 0.73)
...
4. [Support ticket prioritization system](https://github.com/KnightKotOR/SupportTickets-Prioritization):
   ...
   * Built a pipeline with cross-validation and stratified splitting. Trained XGBoost (hyperparameter tuning with Optuna) and achieved **F1 macro = 0.98** (+0.05 compared to a boosting without hyperparameters optimization)
   * Business impact: **eliminated critical errors** (no High incident is predicted as Low)
   * Using treeSHAP demonstrated that the model is not overfitting and relied on real business factors (downtime, number of affected users)
